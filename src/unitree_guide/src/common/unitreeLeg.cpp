...
//足端到对应hip关节处的向量 Forward Kinematics
Vec3 QuadrupedLeg::calcPEe2H(Vec3 q){
// std::cout << "_abadLinkLength: " << _abadLinkLength << std::endl;
...
// std::cout << "pEe2H: " << pEe2H.transpose() << std::endl;

    return pEe2H;
}

// 足端到质心的向量Forward Kinematics
Vec3 QuadrupedLeg::calcPEe2B(Vec3 q){
// std::cout << "calcPEe2H: " << calcPEe2H(q).transpose() << std::endl;
    return _pHip2B + calcPEe2H(q);
}

// 已知关节角、关节角速度->质心速度 Derivative Forward Kinematics
Vec3 QuadrupedLeg::calcVEe(Vec3 q, Vec3 qd){
    return calcJaco(q) * qd;
}

//已知足端向量->关节角 Inverse Kinematics
...
// std::cout << "Jaco: " << std::endl << calcJaco(q) << std::endl;
...
float QuadrupedLeg::q1_ik(float py, float pz, float l1){
    ...
    // q1(0) = atan2(py, -pz) -atan2(l1, sqrt(pow(py,2)+pow(pz,2)-pow(l1,2)));
    // q1(1) = atan2(py, -pz) -atan2(l1, -sqrt(pow(py,2)+pow(pz,2)-pow(l1,2)));
    return q1;
}
...
// Vec2 QuadrupedLeg::q2_ik(Vec2 q1, float q3, float px, float py, float pz, float b3z, float b4z){
//     Vec2 q2;
//     q2(0) = q2_ik(q1(0), q3, px, py, pz, b3z, b4z);
//     q2(1) = q2_ik(q1(1), q3, px, py, pz, b3z, b4z);
//     return q2;
// }