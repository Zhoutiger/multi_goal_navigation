...
Estimator::Estimator(QuadrupedRobot *robotModel, LowlevelState* lowState, 
    ...
    // Qdig.setOnes();
    // Qdig = 0.1*Qdig;
    // Estimator(robotModel, lowState, contact, phase, dt, Qdig);
...
void Estimator::_initSystem(){
    ...
    /* ROS odometry publisher */
        ...
        _pub = _nh.advertise<nav_msgs::Odometry>("odom", 50);
        ...
        //                          0, 0, 0, 0, 1e6, 0, 
        //                          0, 0, 0, 0, 0, 1e-9};
        // _odom_twist_covariance = {1e-9, 0, 0, 0, 0, 0, 
        //                           0, 1e-3, 1e-9, 0, 0, 0, 
        //                           0, 0, 1e6, 0, 0, 0, 
        //                           0, 0, 0, 1e6, 0, 0, 
        //                           0, 0, 0, 0, 1e6, 0, 
...
// std::cout << "_contact: " << _contact->transpose() << std::endl;
...
AvgCov::AvgCov(unsigned int size, std::string name, bool avgOnly, unsigned int showPeriod, unsigned int waitCount, double zoomFactor)
...
void AvgCov::measure(VecX newValue){
    ++_measureCount;
...
// void AvgCov::measure(VecX newValue, MatX covariance){
//     ++_measureCount;
//     for(int i(0); i<_size; ++i){
//         if(covariance(i,i) > 800){
//             newValue(i) = _exp(i);
//         }
//     }
//     if(_measureCount > _waitCount){
...
// void AvgCov::measure(VecX newValue){
//     measure(newValue, _defaultWeight);
// }