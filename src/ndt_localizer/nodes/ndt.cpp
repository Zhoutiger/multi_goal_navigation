...
NdtLocalizer::NdtLocalizer(ros::NodeHandle &nh, ros::NodeHandle &private_nh):nh_(nh), private_nh_(private_nh), tf2_listener_(tf2_buffer_){
  ...
  laser_odom_sub_ = nh_.subscribe("/aft_mapped_to_init", 1, &NdtLocalizer::callback_laser_odom, this);
...
void NdtLocalizer::callback_laser_odom(const nav_msgs::Odometry::ConstPtr & odom_msg_ptr)
{
  geometry_msgs::Quaternion oriention = odom_msg_ptr->pose.pose.orientation;
  geometry_msgs::Point position = odom_msg_ptr->pose.pose.position;
  ...
  laser_to_init_matrix_ = isometry.matrix().cast<float>();
}
...
{
  const auto trans_epsilon = ndt_.getTransformationEpsilon();  //终止条件
  const auto step_size = ndt_.getStepSize();  //最大步长
  const auto resolution = ndt_.getResolution();  //NDT网格分辨率
  const auto max_iterations = ndt_.getMaximumIterations();  //迭代最大次数
  ...
  ndt_new.setInputTarget(map_points_ptr); //第一次扫描的结果 点云配准目标
  // create Thread
  // detach
  ...
  const sensor_msgs::PointCloud2::ConstPtr & sensor_points_sensorTF_msg_ptr)
{
  ...
  if (!init_pose){
    ...
    init_matrix = initial_pose_matrix = initial_pose_affine.matrix().cast<float>();
  ...
  std::cout << laser_to_init_matrix_ << std::endl;
  ndt_.align(*output_cloud, initial_pose_matrix);
  //ndt_.align(*output_cloud, init_matrix * laser_to_init_matrix_);
 // ndt_.align(*output_cloud, laser_to_init_matrix_ * initial_pose_matrix);
 ...
 const Eigen::Matrix4f result_pose_matrix = ndt_.getFinalTransformation();
  ...
  //const Eigen::Matrix4f result_pose_matrix =  laser_to_init_matrix_;
